...
test('admin system history uses its own filtered pagination', () => {
  assert.match(source, /const HISTORY_PAGE_SIZE = 15/);
  assert.match(source, /const pagedHistoryLogs = useMemo/);
  assert.match(source, /const CRUD_PAGE_SIZE = 15/);
  assert.match(source, /const ADMIN_ACCOUNT_PAGE_SIZE = 15/);
  assert.match(source, /const pagedAdminAccounts = useMemo/);
