...
test('Gmail SMTP is preferred and Resend remains a fallback', () => {
  ...
  assert.match(api, /host: "smtp\.gmail\.com"/);
  assert.match(api, /port: 465/);
  assert.match(api, /secure: true/);
  assert.match(api, /minVersion: "TLSv1\.2"/);
  assert.match(api, /@resend\\\.dev/);
  ...
  assert.doesNotMatch(api, /^\s*sender:/m);
