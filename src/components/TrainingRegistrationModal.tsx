...
export default function TrainingRegistrationModal({ clubs, isOpen, onClose }: { clubs: Club[]; isOpen: boolean; onClose: () => void }) {
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault(); if(submitting)return; const normalizedEmail=email.trim().toLowerCase(); if(!isValidGmailAddress(normalizedEmail)){setResult({type:'error',text:'Vui lòng nhập đúng địa chỉ Gmail, ví dụ: tennguoidung@gmail.com.'});return;} setSubmitting(true);setResult(null);try{const r=await fetch('/api/training-registrations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fullName,email:normalizedEmail,clubId,message})});const p=await r.json().catch(()=>({}));if(!r.ok)throw new Error(p.error||'Không thể gửi đăng ký lúc này.');setResult({type:'success',text:'\u0110\u0103ng k\u00fd th\u00e0nh c\u00f4ng! Form s\u1ebd t\u1ef1 \u0111\u00f3ng.'});window.setTimeout(()=>{setFullName('');setEmail('');setClubId('');setMessage('');setResult(null);onClose()}, 3000)}catch(x){setResult({type:'error',text:x instanceof Error?x.message:'Không thể gửi đăng ký.'})}finally{setSubmitting(false)}};
