...
interface VisitorNamePromptProps {
  disabled?: boolean;
}

export default function VisitorNamePrompt({ disabled }: VisitorNamePromptProps) {
  const [visible, setVisible] = useState(false);
  const [visitorName, setVisitorName] = useState('');
  const inputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (disabled || hasCompletedPrompt() || window.location.hash.startsWith('#admin')) return;
  ...
  const skipPrompt = () => {
    void import('../utils/visitorAnalytics')
      .then(({ identifyVisitor }) => identifyVisitor(''))
      .catch(() => {});
    setVisible(false);
  };
  ...
  const submitName = (event: React.FormEvent) => {
    ...
    void import('../utils/visitorAnalytics')
      .then(({ identifyVisitor }) => identifyVisitor(normalizedName))
      .catch(() => {});
    setVisible(false);
  };
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-md animate-in fade-in zoom-in-95 duration-200 overflow-hidden rounded-3xl border border-white/20 bg-white shadow-[0_25px_60px_-15px_rgba(0,35,76,0.35)]">
        ...
        {/* Header gradient màu sắc Vovinam */}
